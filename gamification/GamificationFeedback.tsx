...
const Toast: React.FC<{ event: GamificationEvent, onDismiss: () => void }> = ({ event, onDismiss }) => {
    ...
    return (
        <div className="animate-fade-in-up w-full max-w-sm bg-slate-800/80 backdrop-blur-lg border border-white/20 rounded-2xl shadow-2xl p-4 flex items-center gap-4">
...
const Confetti: React.FC = () => {
    ...
    return (
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-50">
            {particles.map(p => (
                <div key={p.id}
                     className="absolute w-2 h-4 rounded-full animate-fall"
                     style={{ ...p }}
                />
            ))}
            <style>{`
