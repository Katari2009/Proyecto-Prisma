            ...
            <Card className="p-8">
                ...
                <div className="space-y-6 max-h-[50vh] overflow-y-auto pr-2">
                    {questions.map((q, index) => {
                        ...
                        return (
                             <div key={index} className="p-4 rounded-2xl border-2 border-white/10 bg-slate-900/50">
                                <p className="font-semibold mb-3 text-white">{index + 1}. {q.question}</p>
                                <div className="space-y-2">
                                    {q.options.map(option => {
                                        const isUserAnswer = userAnswer === option;
                                        const isCorrectAnswer = q.answer === option;
                                        let optionClass = "border-slate-700 bg-slate-800/50";
                                        if (isCorrectAnswer) {
                                            optionClass = "border-green-500 bg-green-500/20 text-white";
                                        }
                                        if (isUserAnswer && !isCorrect) {
                                            optionClass = "border-red-500 bg-red-500/20 text-slate-300 line-through";
                                        }
                                        return (
                                            <div key={option} className={`p-2 rounded-lg border text-sm ${optionClass}`}>
                                                {option}
                                            </div>
                                        );
                                    })}
                                ...
                                {!isCorrect && (
                                    <div className="mt-3 pt-3 border-t border-white/10">
                                        <p className="text-xs text-slate-300"><span className="font-semibold text-sky-300">Explicación:</span> {q.explanation}</p>
                                    </div>
                                )}
                            </div>
                        );
