...
const MatchingActivity: React.FC<Props> = ({ module, activity, onComplete }) => {
    ...
    const handleSubmit = () => {
        ...
        const score = (totalItems > 0) ? (totalCorrect / totalItems) * 100 : 0;
    ...
    const getBorderColor = (itemId: string) => {
        if (!submitted) return 'border-slate-600';
    ...
    return (
        <Card className="p-8">
            ...
            <Card className="bg-slate-900/50 mb-6 p-4">
                <p className="italic text-slate-300">{currentCase.description}</p>
            </Card>

            <div className="space-y-4 mb-8">
                {currentCase.items.map(item => (
                    <div key={item.id} className="grid grid-cols-2 gap-4 items-center">
                        <div className="font-semibold p-3 bg-slate-700 rounded-lg text-white">
                            {item.text}
                        </div>
                        <select
                            ...
                            disabled={submitted}
                            className={`w-full p-3 rounded-lg bg-slate-800 border-2 transition-colors text-white ${getBorderColor(item.id)}`}
                        >
                            <option value="" disabled>Selecciona una opción...</option>
                            {shuffledTargets.map(target => (
                                <option key={target.id} value={target.id} className="bg-slate-800">{target.text}</option>
                            ))}
                        </select>
                    </div>
                ))}
            ...
            {submitted ? (
                <div className="space-y-4">
                     <p className="text-center font-bold text-white">Revisión Finalizada</p>
